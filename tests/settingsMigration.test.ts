...
describe('loadSettings — versioning', () => {
    ...
    it('switches Search on for an existing config', () => {
        const fresh = load({ settingsVersion: 12, activeModuleIds: ['dashboard'] });
        expect(fresh.activeModuleIds).toContain('search');
